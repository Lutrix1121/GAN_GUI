...
def setupPaths(gui, file_label, path_label):
    ...
    setup_window.configure(bg=current_theme['bg'])
    ...
    title_label = tk.Label(setup_window, text="Configure Data File and Save Location", 
                          font=("Arial", 16, "bold"), bg=current_theme['bg'], fg=current_theme['text'])
    title_label.pack(pady=20)
    globals.WIDGETS.append((title_label, 'label'))
    
    # File selection section
    file_frame = tk.Frame(setup_window, bg=current_theme['bg'])
    file_frame.pack(pady=20, padx=20, fill='x')
    
    file_section_label = tk.Label(file_frame, text="1. Select Data File", 
                                 font=("Arial", 12, "bold"), bg=current_theme['bg'], fg=current_theme['text'])
    ...
    file_button = tk.Button(file_frame, text="Browse for Data File", command=lambda: selectFile(file_status_label, file_label), 
                           font=("Arial", 12), width=20, height=2, bg=current_theme['button_bg'])
    file_button.pack(pady=5)
    
    file_status_label = tk.Label(file_frame, text="No file selected", 
                                font=("Arial", 10), bg=current_theme['bg'], fg='red')
    file_status_label.pack(pady=5)

    # Integer columns section
    integer_frame = tk.Frame(setup_window, bg=current_theme['bg'])
    ...
    integer_info_label = tk.Label(integer_frame, text="Enter column names that should be treated as integers, separated by commas:", 
                                 font=("Arial", 10), bg=current_theme['bg'], fg=current_theme['text'])
    ...
    # Save path section
    path_frame = tk.Frame(setup_window, bg=current_theme['bg'])
    ...
    # Control buttons
    button_frame = tk.Frame(setup_window, bg=current_theme['bg'])
    button_frame.pack(pady=30)
    
    done_button = tk.Button(button_frame, text="Done", 
                           command=lambda: closeSetup(setup_window, integer_entry), 
                           font=("Arial", 12), width=15, height=2, bg=current_theme['generate_bg'])
